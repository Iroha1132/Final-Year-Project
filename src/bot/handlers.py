...
class PhotoEncryptBot:
    def __init__(self, app):
        ...
        os.makedirs("output/ciphertext", exist_ok=True)  # New directory for ciphertext
...
"""
        ...
        # Save encrypted_lsb as ciphertext in a text file
        ciphertext_filename = f"ciphertext_{receiver_id}_{file_timestamp}.txt"
        ciphertext_path = os.path.join(PROJECT_ROOT, "output", "ciphertext", ciphertext_filename)
        with open(ciphertext_path, "w") as file:
            file.write(base64.b64encode(encrypted_lsb).decode('utf-8'))
